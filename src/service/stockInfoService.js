...
async function getBasicInfo(stockId) {
    ...
    try {
        ...
        return {
            ...
            tradePrice: numSeparator(
                strToNum(data.lastClosePrice.value) +
            ...
            roe:
                round2Deci(
                    (100 * strToNum(data.eps.value)) / strToNum(data.bps.value)
                ) + '%'
        };
    } catch (e) {
...
async function getNews(stockName) {
    ...
    try {
        body = (
            await axios.get(newsUrl(stockName), {
                headers: {
...
async function getStockOverview(stockId, date) {
    ...
    promises = [
        getBasicInfo(stockId),
        getReports(stockId, date),
        getPastData(stockId),
        getInvestor(stockId)
    ];
    ...
    if (avgPrice[0].toString() === 'NaN') {
        ...
        stockObj.expYield =
            Math.round(
                (strToNum(stockObj.priceAvg) / strToNum(stockObj.tradePrice) - 1) * 100
            ) + '';
    }
    try {
        stockObj.score = (
            await docClient.query(getScoreQuery(stockId)).promise()
        ).Items[0].score.toString();
    } catch (e) {
        stockObj.score = '-';
    }
